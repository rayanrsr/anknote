...
from anknote.config import load_config, save_config, apply_env_overrides, create_default_config


console = Console()


def setup_logging(verbose: bool = False, quiet: bool = False, log_level: Optional[str] = None) -> None:
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if input_path.is_file() and input_path.suffix not in [".md", ".markdown"]:
        console.print(f"[yellow]Warning:[/yellow] Input file is not a markdown file: {input_path}")
    
    # Validate output path
    output_path = Path(args.input_path if args.in_place else args.output)
    if output_path.is_file():
        console.print(f"[red]Error:[/red] Output path is a file, expected directory: {output_path}")
        sys.exit(1)
...
Configuration:
  ...
  anknote --show-config               # Show current configuration
    ...
    parser.add_argument(
        "-m", "--model",
        help="AI model name in LiteLLM format"
    )
    ...
    parser.add_argument(
        "--max-retries",
        type=int,
        help="Maximum retry attempts for AI generation"
    )
