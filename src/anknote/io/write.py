...
def split_cards(content: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        if not start_match:
            ...
            logger.warning("No START/END markers found, attempting to parse entire content")
        ...
        for card_part in card_parts:
            ...
            try:
                ...
                if prompt and answer:
                    ...
                    logger.warning(f"Skipping card with empty prompt or answer")
...
def generate_flashcards_for_note(
    ...
    for attempt in range(max_retries):
        try:
            logger.debug(f"Generating cards for {note_card.input_path} (attempt {attempt + 1})")
            ...
            logger.warning(f"Attempt {attempt + 1} failed for {note_card.input_path}: {e}")
            if attempt == max_retries - 1:
                logger.error(f"Failed to generate cards for {note_card.input_path} after {max_retries} attempts")
