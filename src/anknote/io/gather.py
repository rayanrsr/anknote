...
def retrieve_input(
    ...
    for nc in cards:
        try:
            ...
            if input_path.is_dir():
                ...
                relative_path = nc.input_path.name
            ...
            if nc.output_path.exists() and not overwrite:
                logger.debug(f"Skipping {nc.input_path} as output already exists: {nc.output_path}")
            else:
    ...
    logger.info(f"Processing {len(valid_cards)} files (skipped {len(cards) - len(valid_cards)})")
    return valid_cards
